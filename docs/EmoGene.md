...
> The implementation details and code can be found in `server/models/GeneFacePlusPlus/emogene`.

## Evaluation Using Py-Feat
We evaluate the emotional expressiveness of generated videos from `EmoGene(ours)` and `GeneFace++` using [Py-Feat](https://github.com/cosanlab/py-feat), a comprehensive facial expression analysis toolkit. 

The dataset of emotional audio input used for evaluation is [RAVDESS](https://www.kaggle.com/datasets/uwrfkaggler/ravdess-emotional-speech-audio), which contains 24 professional actors (12 male, 12 female) performing various emotional expressions, including `neutral`, `happy`, `surprise`, `angry`, `sad`, `fearful`, and `disgust`.

Our evaluation focuses on the Max emotion scores across different emotional categories, comparing the outputs of `EmoGene` and `GeneFace++` with `happiness`, `sadness`, `anger`, `fear`, `disgust`, and `surprise` audio inputs.

The following figure illustrates the comparison of Max emotion scores between `EmoGene(ours)` and `GeneFace++`, demonstrating that our method consistently achieves higher emotional expressiveness across all categories.
![alt text](../assets/emogene/figures/overall_emotion_max_scores_comparison.png)
> The evaluation details and code can be found in `server/models/GeneFacePlusPlus/emogene/evaluation/AU/pyfeat_eval_script.py`.
