...
class VAEModel(nn.Module):
    ...
    def forward(self, batch, ret, train=True, return_latent=False, temperature=1.):
        ...
        # [B, T] 
        # y_mask is the mask for the target sequence
        # y_mask is 1 where there is a target and 0 where there is padding
        
        mel = batch['audio'].to(self.device)
        # [B, T, 1024]
        # audio is the input audio features
        # mel is the mel spectrogram of the audio
        # mel spectrogram means the mel scale of the audio features
        mel = self.downsampler(mel)
        # mel is downsampled by a factor of 2
        cond_feat = self.mel_encoder(mel.transpose(1,2)).transpose(1,2)
        # [B, T, 64]
        # mel is passed through a convolutional layer
        # cond_feat is the output of the convolutional layer
        # cond_feat is the condition feature
        

        if self.cond_drop:
            cond_feat = self.dropout(cond_feat)
            # cond_feat is passed through a dropout layer
            # cond_feat is the output of the dropout layer
        ...
        if not infer:
            ...
            # x_recon is the output of the model
            # x_recon is the predicted target sequence
            # z_p is the latent representation of the input
            # z_p is the latent representation of the target sequence
            # FVAE is the model used for prediction of the target sequence
            # FVAE means Flow-based Variational Autoencoder
            x_recon = x_recon * mask.unsqueeze(-1)
            # x_recon is multiplied by the mask
            # x_recon is the predicted target sequence
            # mask is the mask for the target sequence
            # mask is 1 where there is a target and 0 where there is padding
            
            ret['pred'] = x_recon # predicted target sequence
            # ret is a dictionary
            # x_recon has the data structure of a tensor
            # x_recon.shape is [B, T, 64]
            # B is the batch size
            # T is the target sequence length
            # 64 is the dimension of the target sequence
