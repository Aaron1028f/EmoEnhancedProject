...
def process_results_single_target_actor(results_input, src, actor_id):
    """
    Processes video results from a list of dicts or a DataFrame.
    The data is expected to be in a "flat" format.
    """
    ...
    actor_df = results_df[results_df['actor_id'] == actor_id]
    return actor_df.groupby('emotion').mean()
...
def load_and_print_from_csv_target_actor(emogene_csv_path, genefacepp_csv_path, actor_id):
    """
    Loads raw results from two FLAT CSV files, processes them, and prints the final report.
    """
    print(f"Loading EmoGene results from: {emogene_csv_path}")
    ...
    # Process the loaded DataFrames
    emogene_avg = process_results_single_target_actor(emogene_df, 'emogene', actor_id)
    genefacepp_avg = process_results_single_target_actor(genefacepp_df, 'genefacepp', actor_id)
    print(f"Final results for Actor {actor_id}:")
    # Print the final formatted report
    print_final_result(emogene_avg, genefacepp_avg)
...
def main():
    if RUN_FULL_DETECTION:
        ...
        if os.path.exists(EXISTING_CSV_FILE_PATH_EMOGENE) and os.path.exists(EXISTING_CSV_FILE_PATH_GENEFACEPP):
            ...
            for actor_id in ACTOR_ID_LIST:
                load_and_print_from_csv_target_actor(EXISTING_CSV_FILE_PATH_EMOGENE, EXISTING_CSV_FILE_PATH_GENEFACEPP, actor_id=actor_id)
