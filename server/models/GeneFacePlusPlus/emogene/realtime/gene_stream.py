...
class GeneFace2Infer:
    ...
    # }        
        ...
        if self.postnet_model is not None:
            ...
            feat_fuse, feat_errors, feat_weights = compute_LLE_projection_by_parts(feats=idexp_lm3d, feat_database=idexp_lm3d_ds_lle, K=10, gene_feat=feat_fuse_geneface)
            ...
            # <experiment>
            # analyze the mouth open distance and other landmarks displacement limitations
            from emogene.experiment.lm_displacement_limit.check_lm_limit import analyze_lm_displacement_limit
            mean_face_exp = self.face3d_helper.key_mean_shape[index_lm68_from_lm478].squeeze().reshape([1, -1, 3])
            analyze_lm_displacement_limit(idexp_lm3d_ds_lle, idexp_lm3d, idexp_lm3d_geneface, mean_face_exp)
            # </experiment>
    ...
    # [新增] 用於串流的生成器方法
    @torch.no_grad()
    def stream_secc2video(self, batch, inp=None):
        """
        這是一個生成器函數，用於逐幀渲染並產出結果。
        """
        num_frames = len(batch['poses'])
        rays_o, rays_d, cond_inp, bg_coords, bg_color, poses, lm68s, eye_area_percent = \
            batch['rays_o'], batch['rays_d'], batch['cond_wins'], batch['bg_coords'], \
            batch['bg_img'], batch['poses'], batch['lm68'], batch['eye_area_percent']
        hparams = self.secc2video_hparams

        with torch.cuda.amp.autocast(enabled=True):
            for i in tqdm.trange(num_frames, desc="GeneFace++ is streaming frames..."):
                model_out = self.secc2video_model.render(
                    rays_o[i], rays_d[i], cond_inp[i], bg_coords, poses[i], index=i, staged=False, 
                    bg_color=bg_color, lm68=lm68s[i], perturb=False, force_all_rays=False,
                    T_thresh=inp['raymarching_end_threshold'], eye_area_percent=eye_area_percent[i],
                    **hparams
                )
                
                if self.secc2video_hparams.get('with_sr', False):
                    pred_rgb = model_out['sr_rgb_map'][0].cpu()
                else:
                    pred_rgb = model_out['rgb_map'][0].reshape([512, 512, 3]).permute(2, 0, 1).cpu()

                # 將 Tensor 轉換為適用於 ffmpeg 的 NumPy 陣列 (H, W, C) uint8
                frame_np = (pred_rgb.permute(1, 2, 0).clamp(0, 1) * 255).byte().cpu().numpy()
                
                yield frame_np

    # [新增] 串流推論的完整流程入口
    @torch.no_grad()
    def stream_forward_system(self, batch, inp):
        """
        這是串流推論的完整流程，它是一個生成器。
        """
        # 1. 準備階段：這部分和原來一樣，計算出所有需要的條件
        self.forward_audio2secc(batch, inp)
        
        # 2. 渲染與產出階段：從 stream_secc2video 生成器中逐幀產出
        yield from self.stream_secc2video(batch, inp)
