...
class GeneFace2Infer:
    ...
    def forward_audio2secc(self, batch, inp=None):
        ...
        lm468_bs_np_path = inp['lm468_bs_np_path']
        print('='*20)
        print(lm468_bs_np_path)
        print('='*20)
        # lm468_bs_ = np.load("./_test/emo_out/lm468_bs_01.npy")
        lm468_bs_np = np.load(lm468_bs_np_path)
        lm468_bs_ = torch.from_numpy(lm468_bs_np).float().cuda()
        
        lm468_bs_ = lm468_bs_ * 10
        
        print('='*20)
        print(f'num of frames: {num_frames}')
        print(f'lm468_bs.shape: {lm468_bs_.shape}')
        
        # make lm468_bs.shape[0] == num_frames
        lm468_bs = torch.zeros([num_frames, 468, 3]).to(lm468_bs_.device)
        
        num_frames_30fps = lm468_bs_.shape[0]
        frame_counter = 0
        ...
        for i in range(num_frames_30fps):
            if((i+1) % 6 == 0 or frame_counter >= num_frames):
                ...
                lm468_bs[frame_counter] = lm468_bs_[i]
                frame_counter += 1
                
        print(f'num of frames: {num_frames}')
        print(f'lm468_bs.shape: {lm468_bs.shape}')
        
        print('='*20)

        # ============== bs_ver_modified ==============
        # load lm468_bs (use function call)
        ...
        if self.postnet_model is not None:
            ...
            # ============== bs_ver_modified ==============
            idexp_lm3d = self.face3d_helper.reconstruct_idexp_lm3d(id, exp, bs=lm468_bs)
        ...
        lm2d = self.face3d_helper.reconstruct_lm2d_nerf(id, exp, smo_euler, smo_trans, bs=lm468_bs)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--lm468_bs_np_path", default='./../DATA/lm468_bs_temp/lm468_bs.npy')
