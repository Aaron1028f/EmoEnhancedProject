    ...
    def infer(self, spk_audio_prompt, text, output_path,
        ...
        for seg_idx, sent in enumerate(segments):
            ...
            with torch.no_grad():
                ...
                if is_streaming:
                    # 加上片段間的靜音
                    if seg_idx > 0 and interval_silence > 0:
                        sil_dur = int(sampling_rate * interval_silence / 1000.0)
                        sil_tensor = torch.zeros(1, sil_dur, device='cpu')
                        yield (sampling_rate, sil_tensor.type(torch.int16).numpy().T)

                    # 返回當前片段的音訊
                    wav_data = wav.cpu().type(torch.int16).numpy().T
