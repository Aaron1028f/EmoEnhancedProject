...
LLM_PROVIDER = "gemini" # gemini-2.5-flash 是免費使用的

# --- 模型名稱 ---
GEMINI_MODEL_NAME = "gemini-2.5-flash"
CHATGPT_MODEL_NAME = "gpt-4.1" # "gpt-4.1-mini" 也是一個快速回應的好選擇

# --- 檔案路徑 ---
# 注意：路徑是相對於專案根目錄
FAISS_INDEX_PATH = "data/faiss_index_qa_openai"
JSON_DATA_PATH = "data/QAdata.json"
SYSTEM_PROMPT_FILE = "data/system_prompt.txt"

# --- 功能開關 ---
ENABLE_CONVERSATIONAL_MEMORY = True # 是否啟用對話記憶
ENABLE_DEBUG_MODE = False           # 是否啟用除錯模式，會印出詳細的 prompt 內容
ENABLE_COT_MODE = False             # 是否啟用思維鏈 (Chain of Thought) 模式 (僅在啟用對話記憶時有效)

# --- 人格設定 ---
ROLE_NAME = "方識欽"

# --- 系統 Prompt 模板 ---
# 這裡直接讀取檔案內容，以便在 services.py 中格式化
# 這樣可以保持 config.py 的簡潔
try:
    with open(SYSTEM_PROMPT_FILE, "r", encoding="utf-8") as f:
        role_description_content = f.read().strip()
except FileNotFoundError:
    role_description_content = "錯誤：找不到人格設定檔。"
    print(f"警告：在 config.py 中找不到人格設定檔 '{SYSTEM_PROMPT_FILE}'。")
