...
# 1. 常量設定
# API_KEY = os.getenv("OPENAI_API_KEY") or "你的金鑰"
EMBED_MODEL = "text-embedding-ada-002"
INDEX_PATH  = "faiss_index"         # 資料夾或檔名前綴
DOCS_PATH   = "QAdata.json"

# 2. 初始化 Client & Embeddings
client = OpenAI(api_key=OPENAI_API_KEY)
emb   = OpenAIEmbeddings(openai_api_key=OPENAI_API_KEY, model=EMBED_MODEL)

# 3. 如果已存在 index，就直接載入；否則才重新 build
if os.path.exists(f"{INDEX_PATH}/index.faiss"):
    # 載入 FAISS 索引，設定 allow_dangerous_deserialization=True 以允許反序列化
    ...
    # 3.1 讀 JSON 並 build Document list
